...
function withToolInstructions(system: string, maxTokens: number) {
  return [
    ...
    "Web access is available through web_search, web_read_url, web_fetch_url, web_extract_links, and web_fetch_many. web_search is a no-key prototype that returns ranked candidate URLs and untrusted snippets only; use fetch tools to read sources before relying on them. For long pages, prefer web_read_url because it returns one bounded text window plus next_offset for continuing instead of flooding the turn.",
